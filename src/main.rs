...
fn main() {
    ...
    let worker_handles: Vec<_> = (0..NUM_WORKERS as u64).map(|id| {
        let (p, m, s, tx, c) = (
            Arc::clone(&pool),
            Arc::clone(&metrics),
            Arc::clone(&stop_flag),
    ...
    println!("  Injecting {} players at {}/sec...", TOTAL_PLAYERS, PLAYERS_PER_SECOND);
    run_simulation(
        Arc::clone(&pool),
        Arc::clone(&metrics),
        TOTAL_PLAYERS,
    ...
    loop {
        ...
        if sz < 10 { println!("  Pool drained ({} left).", sz); break; }
        if drain_start.elapsed().as_secs() > 30 {
            println!("  Drain timeout — {} players still waiting.", sz); break;
        }
    ...
    if leftover > 0 {
        println!("  ℹ  {} players remain (< 10, can't form a full game)", leftover);
    } else {
...
fn print_match_result(gm: &GameMatch) {
    ...
    println!(
        ...
        gm.id, avg(&gm.team_a), avg(&gm.team_b), gm.quality_score, bar, gm.avg_wait_ms
    );
}
