...
pub struct GameMatch {
    ...
    pub avg_wait_ms: f64,    //Average wait time of all players in this match (in seconds)
}

impl GameMatch {
    pub fn new(id: u64, team_a: Vec<Player>, team_b: Vec<Player>) -> Self {
        let quality_score = compute_quality(&team_a, &team_b);
...
mod tests {
    ...
    fn test_perfect_balance_gets_high_score() {
        ...
        assert!(
            game_match.quality_score > 0.90,
            "Expected high quality score, got {}",
            game_match.quality_score
        );
    }
    ...
    fn test_unbalanced_teams_get_low_score() {
        ...
        assert!(
            game_match.quality_score < 0.40,
            "Expected low quality score, got {}",
            game_match.quality_score
        );
    }
}
